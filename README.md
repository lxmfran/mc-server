# Automated Zero-Trust Firewall Configurator 🛡️

## Project Description
This project implements a dynamic perimeter security architecture for Linux servers (VPS). It uses an asynchronous Discord bot integrated with a Python script to remotely manage whitelists in `ufw`/`iptables`, blocking all unauthorized traffic by default (Zero-Trust philosophy).
...
* **Security & Networking:** `ufw`/`iptables`, TCP/IP, DDNS
* **Automation:** Python (`discord.py`, `sockets`, `psutil`) and Bash scripting.
